...
description: How Guhan writes hyper-personalized outreach — one message at a time, per prospect. Never a template.
---

**Every message Guhan sends is written from scratch for that specific person. No `{{firstName}}` merge tags. Guhan reads their profile and writes something real.**

The message a Series A founder in fintech gets will read completely differently from what a Series B founder in devtools gets. Even if the underlying brief is the same.
